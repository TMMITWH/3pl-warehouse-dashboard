...
(async function () {
  ...
  function printReport() {
    ...
    // numCols = array of column indexes that hold numbers (right-aligned,
    // formatted). Header, body and footer all use the SAME per-column class
    // so everything lines up under its heading.
    const cell = (tag, c, isNum) =>
      `<${tag} class="${isNum ? "n" : ""}">` +
      (typeof c === "number"
        ? c.toLocaleString("en-US", { maximumFractionDigits: 3 })
        : esc(c)) +
      `</${tag}>`;
    const tbl = (headers, rows, foot, numCols = [], widths = []) => {
      const isNum = (i) => numCols.includes(i);
      const cols = headers.map((_, i) =>
        widths[i] ? `<col style="width:${widths[i]}%">` : `<col>`).join("");
      return `<table><colgroup>${cols}</colgroup>` +
        `<thead><tr>${headers.map((h, i) => cell("th", h, isNum(i))).join("")}</tr></thead>` +
        `<tbody>${rows.map((r) =>
          `<tr>${r.map((c, i) => cell("td", c, isNum(i))).join("")}</tr>`).join("")}</tbody>` +
        (foot ? `<tfoot><tr>${foot.map((c, i) => cell("td", c, isNum(i))).join("")}</tr></tfoot>` : "") +
        `</table>`;
    };
...
<style>
  ...
  td { padding: 3px 6px; border-bottom: 0.5px solid #ccc; word-wrap: break-word; overflow-wrap: break-word; }
